...
CREATE TABLE app_fp.proposal_type (
    code                   varchar(20) NOT NULL,
    description            varchar(100) NOT NULL,
    CONSTRAINT proposal_type_code_pk PRIMARY KEY (code)
)
;
COMMENT ON TABLE app_fp.proposal_type IS 'Code table for proposal types.'
;

INSERT INTO app_fp.proposal_type (code, description) VALUES 
('INITIAL', 'Initial'),
('FINAL', 'Final')
;
...
;
ALTER TABLE app_fp.fop_proposal ADD CONSTRAINT fop_proposal_proposal_type_code_fk
    FOREIGN KEY (proposal_type_code)
    REFERENCES app_fp.proposal_type(code)
;
...
;
COMMENT ON COLUMN app_fp.fop_proposal.proposal_type_code IS 'Type of proposal.'
;
...
;
COMMENT ON COLUMN app_fp.fop_holder.client_id IS 'Forest Client Number from the Client system.'
;
...
CREATE TABLE app_fp.fop_district (
    fop_district_id bigint GENERATED BY DEFAULT AS IDENTITY (START WITH 1000 INCREMENT BY 1),
    fop_id          bigint NOT NULL,
    ...
    update_user     varchar(30),
    update_date     TIMESTAMP(6) WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fop_district_fop_id_fk FOREIGN KEY (fop_id) REFERENCES app_fp.fop(fop_id)
)
;

CREATE INDEX idx_fop_district_fop_id ON app_fp.fop_district(fop_id);

COMMENT ON TABLE app_fp.fop_district IS 'District details for a specific Forest Operations Plan.'
;
COMMENT ON COLUMN app_fp.fop_district.fop_district_id IS 'Automatically generated key used to identify the uniqueness of a district record.'
;
COMMENT ON COLUMN app_fp.fop_district.fop_id IS 'Foreign key referencing the FOP table.'
;
COMMENT ON COLUMN app_fp.fop_district.district_code IS 'District organization code from ORG_UNIT table.'
;
COMMENT ON COLUMN app_fp.fop_district.revision_count IS 'Track version for optimistic locking.'
;
COMMENT ON COLUMN app_fp.fop_district.create_user IS 'The user or proxy account that created the record.'
;
COMMENT ON COLUMN app_fp.fop_district.create_date IS 'The date and time the record was created.'
;
COMMENT ON COLUMN app_fp.fop_district.update_user IS 'The user or proxy account that created or last updated the record.'
;
COMMENT ON COLUMN app_fp.fop_district.update_date IS 'The date and time the record was created or last updated.'
;
...
ALTER TABLE app_fp.cutblock ADD CONSTRAINT cutblock_fop_proposal_id_fk
    ...
    REFERENCES app_fp.fop_proposal(fop_proposal_id)
;
ALTER TABLE app_fp.cutblock ADD CONSTRAINT cutblock_fop_holder_id_fk
    FOREIGN KEY (fop_holder_id)
    REFERENCES app_fp.fop_holder(fop_holder_id)
;
...
;
COMMENT ON COLUMN app_fp.cutblock.fop_holder_id IS 'Foreign key referencing the FOP holder table.'
;
...
;
COMMENT ON COLUMN app_fp.cutblock.development_date IS 'The date when the cutblock development is planned or occurred.' 
;
